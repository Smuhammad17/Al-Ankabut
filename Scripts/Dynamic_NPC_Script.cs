...
public class Dynamic_NPC_Script : MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        if (Dynamic_NPC.Resurrected && NPCName.GetComponent<TextMesh>().text != "<color=gold>Resurrected</color>")
        {
        ...
        //look at player
        if (!Talking && playerDist < 45) 
        ObjBase.transform.LookAt(PlayerOBJ.transform.Find("PlayerViewport").transform.position);

       


        if (playerDist > 15 && PreviewBox.activeSelf)
        {
            NPCName.SetActive(false);
            PreviewBox.SetActive(false);
        }

        else if (playerDist < 15 && !PreviewBox.activeSelf)
        {
        ...
        if (Talking)
        {
            ...
            if(HCanv.GetComponent<Canvas>().enabled != false)
            HCanv.GetComponent<Canvas>().enabled = false;


            if (PlayerOBJ.transform.Find("OriginalManModel").GetComponent<SpriteRenderer>().enabled)
            ...
            if (!Dynamic_NPC.Resurrected)
            {
                ...
                if (Input.GetKeyDown("e") && !GameObject.FindGameObjectWithTag("MathChallenge").GetComponent<MathChallenge>().GameActive)
                {
                    if (x < Dynamic_NPC.Dialogues.Length - 1)
                    {
                        ...
                        DiaCanvas.GetComponent<AudioSource>().pitch = UnityEngine.Random.Range(0.90f, 1.1f);
                ...
                if (Input.GetKeyDown("q") && !GameObject.FindGameObjectWithTag("MathChallenge").GetComponent<MathChallenge>().GameActive)
                {
                    if (x >= 1)
                    {
                        //stop typing sentece if player skips dialogue. 
        ...
        if (PlayerInArea)
        {
            if (Input.GetKeyDown("e") && !Dynamic_NPC.SpiderInDisguise && !Talking && ToTalkAgain && !GameObject.FindGameObjectWithTag("MathChallenge").GetComponent<MathChallenge>().GameActive)
            {
                ToTalkAgain = false;
    ...
    bool haveexited = true;

private void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.tag == "TalkNPC" && haveexited)
        {
            haveexited = false;
